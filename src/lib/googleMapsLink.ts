...
/**
 * Google Maps paylaşım linkinden veya "lat,lng" metninden koordinat çıkarır.
 * Kısa link (maps.app.goo.gl) tarayıcıda CORS yüzünden açılamaz;
 * kullanıcıya uzun URL'yi (adres çubuğundan) yapıştırmasını söyleriz.
 */
export function parseMapsLink(input: string): ParsedMapsCoords | null {
  ...
  // data=...3dLAT!4dLNG already covered; also %2C encoded commas in q=
...
export async function resolveMapsLinkToPlace(
  input: string,
): Promise<GeocodeHit> {
  if (isLikelyShortMapsLink(input)) {
    throw new Error(
      'Kısa link (maps.app.goo.gl) tarayıcıda okunamıyor. Linki tarayıcıda aç → adres çubuğundaki uzun URL’yi kopyala → buraya yapıştır.',
    )
  }
  ...
  if (!parsed) {
    throw new Error(
      'Google Maps konumunu çıkaramadım. Maps’te pin koy → Paylaş → Bağlantıyı kopyala (uzun URL) veya “41.01, 28.97” yaz.',
    )
  }
